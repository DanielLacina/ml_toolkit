    ...
    for i in 0..m_copy.len() {
        ...
        for j in 0..m_copy.len() {
            if j != i {
                ...
                m_copy[j] =
                    add_vectors(&m_copy[j], &multiply_vector_by_scalar(&m_copy[i], -factor));
                identity_m[j] = add_vectors(
                    &identity_m[j],
                    &multiply_vector_by_scalar(&identity_m[i], -factor),
                );
            }
        }
    }
...
mod tests {
    ...
    fn test_multiply_matrix_rectangular() {
        let m1: Vec<Vec<f32>> = vec![vec![1.0, 2.0], vec![3.0, 4.0], vec![5.0, 6.0]];

        let m2: Vec<Vec<f32>> = vec![vec![7.0, 8.0, 9.0], vec![10.0, 11.0, 12.0]];
    ...
    fn test_multiply_matrix_opposite_dimensions() {
        let m1: Vec<Vec<f32>> = vec![vec![1.0, 2.0, 3.0], vec![4.0, 5.0, 6.0]];

        let m2: Vec<Vec<f32>> = vec![vec![7.0, 8.0], vec![9.0, 10.0], vec![11.0, 12.0]];

        let expected_m: Vec<Vec<f32>> = vec![vec![58.0, 64.0], vec![139.0, 154.0]];
    ...
    fn test_inverse_matrix() {
        ...
        assert!(
            zip(expected_m.iter(), inverse_m.iter())
                .all(|(v1, v2)| zip(v1, v2).all(|(c1, c2)| (c1 - c2).abs() < 0.01))
        );
        let expected_identity_matrix = multiply_matrices(&inverse_m, &m);
        assert!(expected_identity_matrix.iter().enumerate().all(|(i, v)| {
            v.iter().enumerate().all(|(j, c)| {
                if i == j {
                    (c - 1.0).abs() < 0.01
                } else {
                    (c - 0.0).abs() < 0.01
